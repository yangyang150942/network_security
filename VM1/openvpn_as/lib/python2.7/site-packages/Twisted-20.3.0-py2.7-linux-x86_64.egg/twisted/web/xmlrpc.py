...
# begin -- defusedxml monkey-patch. See [AS-1131] DoS on /RPC2.
try:
    from defusedxml import xmlrpc as defused_xmlrpc
    xmlrpclib.FastParser = defused_xmlrpc.DefusedExpatParser
    xmlrpclib.GzipDecodedResponse = defused_xmlrpc.DefusedGzipDecodedResponse
    xmlrpclib.gzip_decode = defused_xmlrpc.defused_gzip_decode
    if xmlrpc_server:
        xmlrpc_server.gzip_decode = defused_xmlrpc.defused_gzip_decode
except:
    pass
# end -- monkey-patch
...
try:
    from pyovpn.util.types import myenc
except:
    myenc = lambda x : x
...
class XMLRPC(resource.Resource):
    ...
    def _cbRender(self, result, request, responseFailed=None):
        ...
        try:
            ...
            self._log.failure('')
        if not request._disconnected: # JY ADDED
            request.finish()
