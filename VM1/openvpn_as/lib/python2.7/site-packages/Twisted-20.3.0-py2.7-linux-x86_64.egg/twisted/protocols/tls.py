...
class TLSMemoryBIOProtocol(ProtocolWrapper):
    ...
    def makeConnection(self, transport):
        ...
        if hasattr(transport, 'tlsext_host_name'):  # JY ADDED for httpcli.py
            self._tlsConnection.set_tlsext_host_name(transport.tlsext_host_name)
        if hasattr(transport, 'app_data'):  # JY ADDED for custom SSL verify callback
            self._tlsConnection.set_app_data(self.app_data)
