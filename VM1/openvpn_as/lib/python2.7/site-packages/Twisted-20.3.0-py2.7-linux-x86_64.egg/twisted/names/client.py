...
class Resolver(common.ResolverBase):
    ...
    def parseConfig(self, resolvConf):
        ...
        # JY modified -- use Google DNS if no servers
        if not servers:
            servers.append(('8.8.8.8', dns.PORT))
            servers.append(('8.8.4.4', dns.PORT))


        self.dynServers = servers
        # JY added -- filter out IPv6 addresses
        import re
        re_ipv4 = re.compile(r"^(?:(?:[0-9]|[1-9][0-9]|1[0-9]{2}|2[0-4][0-9]|25[0-5])\.){3}(?:[0-9]|[1-9][0-9]|1[0-9]{2}|2[0-4][0-9]|25[0-5])$")
        self.dynServers = [ (addr, port) for addr, port in self.dynServers if re.match(re_ipv4, addr)]
