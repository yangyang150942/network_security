...
# JY Modified -- should not be fatal, except in the case of another twistd server running
def checkPID(pidfile):
    ...
    if os.path.exists(pidfile):
        try:
            ...
            log.msg('Note: Pidfile %s is empty or contains non-numeric value' % pidfile, isError=True)
            return
        try:
            os.kill(pid, 0)
            ...
            sys.exit("Error: Another twistd server is possibly running, PID=%s (if this is not the case, please delete '%s' and restart)" % (pid, pidfile))            
...
class UnixAppLogger(app.AppLogger):
    ...
    def _getLogObserver(self):
        ...
        UnixAppLogger.log_file = logFile # JY ADDED
